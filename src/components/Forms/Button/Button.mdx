...
Use `buttonShape` to choose the corner treatment:

- `circle` uses fully rounded corners and is the default, preserving the original Button appearance.
- `rounded` uses a medium corner radius.
- `square` uses straight corners.

<Canvas of={HansButtonStories.Shapes} />

Web Component consumers can use the same contract with
`<hans-button button-shape="circle">`.
...
Use `hoverButtonColor` and `hoverButtonVariant` to opt into a dedicated hover style
without changing the resting appearance of the button.

<Canvas of={HansButtonStories.HoverStates} />

The `inverse` variant is useful for white surfaces with semantic-colored text,
especially when paired with a stronger hover state.

## Loading

Use `loading` to render the button itself as a skeleton placeholder.

<Canvas of={HansButtonStories.Loading} />
