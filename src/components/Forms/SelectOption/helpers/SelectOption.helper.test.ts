...
describe('SelectOption.helper', () => {
  ...
  it('Should resolve initial dropdown value', () => {
    expect(getInitialSelectOptionValue('x', undefined, false)).toBe('x');
    expect(getInitialSelectOptionValue(undefined, ['x'], true)).toEqual(['x']);
    expect(getInitialSelectOptionValue(undefined, undefined, true)).toEqual([]);
    expect(getInitialSelectOptionValue(undefined, undefined, false)).toBe('');
  });
  ...
  it('Should create open/toggle handlers', () => {
    ...
    createHandleOpen(setSelectOptionOpen)();
    expect(setIsOpen).toHaveBeenCalledWith(true);

    const toggle = createHandleToggle(setSelectOptionOpen, () => true);
    toggle();
