...
describe('HansInput', () => {
  ...
  it('Should render a multiline textarea with rows and preserve line breaks', () => {
    render(
      <HansInput
        inputType="textarea"
        textareaRows={7}
  ...
  it('Should apply formatting from the optional textarea toolbar', () => {
    const onValueChange = vi.fn();
    render(
      <HansInput
        inputType="textarea"
        ...
        boldActionLabel="Make bold"
        italicActionLabel="Make italic"
        underlineActionLabel="Underline text"
        unorderedListActionLabel="Create list"
    ...
    expect(onValueChange).toHaveBeenCalledWith('**Selected** text');
    expect(screen.getByRole('button', { name: 'Make italic' })).toBeEnabled();
    expect(screen.getByRole('button', { name: 'Underline text' })).toBeEnabled();
    expect(screen.getByRole('button', { name: 'Create list' })).toBeEnabled();
  });

  it('Should disable textarea formatting actions with the field', () => {
    render(<HansInput inputType="textarea" formattingToolbar disabled />);
